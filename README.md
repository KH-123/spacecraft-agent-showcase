...
```
SPACECRAFT_AGENT_DEMO/
├── app.py                      # Streamlit 主应用入口（~1790 行）
├── ui_helpers.py               # Streamlit UI 渲染辅助（~2220 行）
├── requirements.txt            # 依赖包列表
├── .env.example                # 环境变量配置示例
├── AGENTS.md                   # Agent 规则定义
├── demo_cases/                 # GitHub / 面试展示案例
│
├── agents/                     # Agent 模块
│   ├── llm_extractor.py        # LLM 参数提取（OpenAI-compatible）
│   ├── parser.py               # 规则参数解析（正则 fallback）
│   ├── normalizer.py           # 参数归一化（单位转换、轨道类型标准化）
│   ├── validator.py            # 硬阈值校验（pass/warning/severe）
│   ├── orbit_interpreter.py    # 轨道参数推断（e=0、SMA、周期、SSO 倾角）
│   ├── orbit_consistency.py    # 轨道物理一致性检查（8 项检查）
│   ├── parameter_inference.py  # 缺失参数 LLM/规则推断
│   ├── planner.py              # 任务规划与工具调度
│   ├── report_generator.py     # Markdown 报告生成
│   ├── design_advisor.py       # RAG-enhanced LLM 设计评审助手
│   ├── rag_retriever.py        # 本地 Markdown 关键词检索
│   ├── mission_context_extractor.py  # 任务上下文提取
│   ├── mission_interpreter.py        # 任务级需求理解
│   ├── design_draft_generator.py     # 候选草案生成
│   ├── llm_estimator.py        # LLM 概念级估算 fallback
│   └── extractor.py            # 提取编排器
│
├── tools/                      # 确定性工程计算工具
│   ├── orbit.py                # 轨道周期、轨道速度
│   ├── power.py                # 太阳能电池阵面积、电池容量
│   └── mass.py                 # 质量预算估算
│
├── docs/                       # 文档
│   ├── project_status.md       # 项目状态
│   ├── change_log.md           # 变更日志
│   ├── interview_technical_brief.md  # 面试技术说明
│   ├── technical_doc_outline_for_interview.md  # 面试文档大纲
│   ├── handoff_to_codex.md     # Codex 交接文档
│   ├── handoff_to_roo.md       # Roo Code 交接文档
│   ├── demo_cases.md           # Demo 展示案例
│   └── rag_knowledge/          # RAG 知识库（Markdown 文件）
│       ├── 01_conceptual_design_limitations.md
│       ├── 02_orbit_design_guidelines.md
│       ├── 03_payload_design_knowledge.md
│       └── 06_design_review_checklist.md
│
├── scripts/                    # 测试脚本
│   ├── test_parameter_flow_refactor.py
│   ├── test_orbit_intelligence.py
│   ├── test_circular_orbit_semantics.py
│   ├── test_concept_agent_flow.py
│   ├── test_extraction.py
│   ├── test_ui_modes.py
│   ├── test_mission_phase2.py
│   └── test_design_state_confirmation_smoke.py
│
├── .roo/rules/                 # Roo Code 规则
│   ├── 01-project-overview.md
│   ├── 02-agent-workflow.md
│   ├── 03-engineering-rules.md
│   └── 04-coding-rules.md
│
└── outputs/reports/            # 生成的报告文件
```
...
> **注意**：Windows 下 `python -m py_compile app.py agents/*.py` 通配符展开失败，请使用 PowerShell：
> ```powershell
